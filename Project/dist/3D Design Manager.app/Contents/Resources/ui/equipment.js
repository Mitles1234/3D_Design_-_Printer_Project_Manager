...
function openFilamentModal(filament = null) {
    ...
    document.getElementById('filament-remove').style.display = filament ? 'inline-flex' : 'none';
...
async function removeFilamentFromModal() {
    ...
    if (!api) return;
    try {
        await api.REMOVE_FILAMENT(activeFilamentId);
        showToast('Filament removed');
        closeFilamentModal();
        await loadData();
    } catch (error) {
        showToast(error?.message || 'Failed to remove filament', true);
    }
}
